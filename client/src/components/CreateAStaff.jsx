...
const CreateAStaff = () => {
    ...
    const handleSignup = async () => {
        const roomsInputElements = document.querySelectorAll('.roomsAssign');
        const roomsUnder = [];
        roomsInputElements.forEach((roomInput) => {
            if (roomInput.value.length > 0)
                roomsUnder.push(roomInput.value);
        })
        const res = await addStaff({ ...data, roomsUnder })
        if (res) {
            setData({ name: "", email: "", password: "", confirmPassword: "" });
            rooms.current.innerHTML = "";
            addInputForRoom();
        }
    }
    const addInputForRoom = () => {
        const newInput = document.createElement('input');
        newInput.className = 'roomsAssign px-2 py-1 border-2 border-black rounded-md w-20 h-12 mt-1';
            ...
            <div className='flex flex-col mt-4 pt-1 pb-4 sm:px-12 sm:py-4 space-y-4 text-lg sm:text-2xl to-red-500 w-[55vw]'>
                ...
                <div className='input flex flex-col space-y-2'>
                    ...
                    <div ref={rooms} className="rooms flex space-x-1 flex-wrap">
                        <input type="number" className='roomsAssign px-2 py-1 border-2 border-black rounded-md w-20 h-12 mt-1' />
                    </div>
                </div>
